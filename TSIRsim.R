...
pomp(London_cases,times="times",t0=1944,
     ...
     params = c(m=13, B1 = 1.285935e-05, B2 = 1.214533e-05, B3 = 1.464063e-05, B4 = 1.277254e-05, B5 = 1.236623e-05, B6 = 1.181611e-05,
                ...
                alpha = 9.70e-01 , Sbar = 114281.6, sd = 628.4102),
...
TSIRsim %>%
  ...
  mutate(data=sim=="data") %>%
  ddply(~time+data,summarize,
        p=c(0.20,0.5,0.65),q=quantile(cases,prob=p,names=FALSE)) %>%
  mutate(p=mapvalues(p,from=c(0.20,0.5,0.65),to=c("lo","med","hi")),
         data=mapvalues(data,from=c(TRUE,FALSE),to=c("data","simulation"))) %>%
  dcast(time+data~p,value.var='q') %>%
  ggplot(aes(x=time,y=med,color=data,fill=data,ymin=lo,ymax=hi))+
  geom_ribbon(alpha=0.2)+ theme_bw() + ylab("cases") + ggtitle("TSIR cases with Data")
...
#Mean Plotting 
##########################################################
simulate(TSIRsim,params=coef(TSIRsim),nsim=100,as.data.frame=TRUE,include.data=TRUE)%>%
  subset(sim!="data")%>%
...
firstFit <- mif2(TSIRsim, Nmif = 150, Np = 1000, #10 was 10000
                 rw.sd = rw.sd(
                   m=0.01,sd = 0.01, alpha=0.01, Sbar=0.01,
...
plot(simulate(TSIRsim, params=coef(firstFit)))
